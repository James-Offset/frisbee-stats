...
class Team():
    ...
    def build_team_stats_page(self, tab_name):
        ...
        self.player_stats_pages[tab_name].pack(fill=tk.BOTH, expand=True)
    ...
    def _create_team_section(self, tab_name):
        ...
        self.team_frame[tab_name] = tk.Frame(self.player_stats_pages[tab_name])
        self.team_frame[tab_name].pack(fill=tk.X, expand=True)
        ...
        for heading in self.team_gui_headings:
            ...
            if column_number in self.tf_separator_columns:
                ...
                self.tf_separator_elements[str(column_number)] = ttk.Separator(self.team_frame[tab_name], orient="vertical")
                self.tf_separator_elements[str(column_number)].grid(row=0 , rowspan=1, column = column_number, sticky='ns', padx=2)
            ...
            # add heading labels
            self.team_gui_headings[heading] = tk.Label(self.team_frame[tab_name], text=heading, font=('Arial', 18))
            self.team_gui_headings[heading].grid(row=0 , column = column_number, sticky=tk.W + tk.E, pady=10)
    ...
    def _create_player_section(self, tab_name):    
        ...
        for heading in self.player_gui_headings:
            ...
            if column_number in self.pf_separator_columns:
                ...
                self.pf_separator_elements[str(column_number)] = ttk.Separator(self.player_frame[tab_name], orient="vertical")
                self.pf_separator_elements[str(column_number)].grid(row=0 , rowspan=1, column = column_number, sticky='ns', padx=2)
            ...
            # add heading labels
            self.player_gui_headings[heading] = tk.Label(self.player_frame[tab_name], text=heading, font=('Arial', 18))
            self.player_gui_headings[heading].grid(row=0 , column = column_number, sticky=tk.W + tk.E, pady=10)
    ...
    def new_player_entry(self, player_name, player_number):
        ...
        for tab_name in self.player_stats_pages:
            ...
            # redraw separators
            for separator in self.pf_separator_elements:
                self.pf_separator_elements[separator].grid(row=0 , rowspan=self.display_row_number+1, column = int(separator), sticky='ns', padx=2)
